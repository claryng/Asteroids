...
public abstract class Asteroids implements AnimatedObject {
     ...
     * 
     * @return the x coordinate of the point where we want the asteroid to fly
     *         to
     */
     ...
     * 
     * @return the y coordinate of the point where we want the asteroid to fly
     *         to
     */
    ...
    /**
     * Randomly set the angle with which a large asteroid will fly into the
     * screen based on the edge where the asteroid starts
     * 
    ...
    public double setRandomAngle() {
        ...
        if (targetedY == 570) {
            ...
            if (targetedX < 300) {
                angle = Math.atan(Math.abs((getLocationY() - getTargetedY()))
                        / Math.abs((getLocationX() - getTargetedX())));
                // Right side of bottom edge
            } else {
                angle = Math.atan(Math.abs((getLocationY() - getTargetedY()))
                        / ((-1) * Math.abs((getLocationX() - getTargetedX()))));
            }
            ...
            if (targetedX < 300) {
                angle = Math.PI / 2 + (Math.atan(Math
                        .abs((getLocationY() - getTargetedY()))
                        / (Math.abs((getLocationX() - getTargetedX())))));
                // Right side of upper edge
            } else {
                angle = Math
                        .atan(Math.abs((getLocationY() - getTargetedY()))
                                / (Math.abs((getLocationX() - getTargetedX()))))
                        - Math.PI;
            }
    ...
    public void move() {
        ...
        if (!isHit) {
            ...
            setLocationX(this.getLocationX()
                    + 1 * ((this.getTargetedX() - this.getLocationX())));
            setLocationY(this.getLocationY()
                    + 1 * ((this.getTargetedY() - this.getLocationY())));

            // Wrap the asteroid around the screen
            setLocationX(
                    (this.getLocationX() <= 0) ? WIDTH + this.getLocationX()
                            : this.getLocationX() % WIDTH);
            setLocationY(
                    (this.getLocationY() <= 0) ? WIDTH + this.getLocationY()
                            : this.getLocationY() % WIDTH);
    ...
    /**
     * Randomly choose x and y coordinates of a large asteroid's starting point
     * and set the x and y coordinates of the point the asteroid will fly to on
     * the screen Asteroid must fly in from outside of screen
     */
    public void setRandom() {
        ...
        if (locationX < 0 || locationX > 600) {
            ...
            // If starting x coordinate is in the screen, starting y coordinate
            // must be outside of screen
        } else if (locationX >= 0) {
    ...
    /**
     * Returns the shape of the asteroid after applying the current translation
     * and rotation
     * 
    ...
    /**
     * Split an asteroid into smaller asteroids if hit by a shot Each large
     * asteroid is split into two medium asteroids Each medium asteroid is split
     * into two smaller asteroids Smaller asteroids move at the same speed.
     * Speed is offset by +/- pi/4 radians
     * 
    ...
    public void split(double angle, double x, double y) {
        ...
        if (this.getClass() == LargeAsteroids.class) {
            Asteroids a = new MediumAsteroids(animation, angle - Math.PI / 4, x,
                    y);
            Asteroids b = new MediumAsteroids(animation, angle + Math.PI / 4, x,
                    y);
            ...
            Asteroids a = new SmallAsteroids(animation, angle - Math.PI / 4, x,
                    y);
            Asteroids b = new SmallAsteroids(animation, angle + Math.PI / 4, x,
                    y);
